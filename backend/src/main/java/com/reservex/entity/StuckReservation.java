 ...
 * 桶 key(要 INCR 回补哪个桶)和 dup key(要 DEL 哪个配额位)。这两个值
 * **只存在于 Redis 的 occupy 载荷里**。occupy 现已刻意无 TTL 正是为此
 * (它是扣减后的恢复证据,只能由成功落库的消费者或补偿脚本删除),
 * 但"单点 Redis 丢数据/被误删/策略配成 allkeys-lru"依然会让这两个参数消失 ——
 * 那时人工连"该回滚什么"都查不出来,桶余量**永久泄漏**:该场次永远少几个名额,
 * 且对账每轮都报同一个 diff 而无从修复。所以转人工的当下必须把它们**抄进持久存储**,
 * 不把可用性押在 Redis 上。
 *
