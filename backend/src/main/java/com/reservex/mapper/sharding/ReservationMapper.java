 ...
 *       <b>但不能在抢号这类高频路径上这么查。</b></li>
   *   <li>过期候选扫描({@code WHERE status=0 AND valid_until&lt;now})同样广播,
   *       这也是可接受的:它只投递定时 timeout 消息,不在用户等待路径上。</li>
 * </ul>
 *
 * <p>用户取消和核销都带 {@code version} 做最终 CAS；窗口期取消先在 occupy 上按
 * version=0 标记，消费者落库后的收口仍以新建行的 version=0 更新。
 */
public interface ReservationMapper extends BaseMapper<Reservation> {
     ...
     *
     * <p>{@code version} 对应 HTTP If-Match；{@code valid_until >= now} 守卫防
     * "场次已结束还能取消"。
     *
     * @return 0 表示状态已变(重复取消/已核销/已过期),调用方据此返对应错误码
     */
    int cancelByNo(@Param("userId") Long userId,
     ...
     */
    int verifyByNo(@Param("userId") Long userId,
                   @Param("reservationNo") Long reservationNo,
                   @Param("version") Integer version,
                   @Param("verifiedAt") LocalDateTime verifiedAt);

    /** 扫描已到期的 RESERVED，交给 timeout 消费者逐条收口。 */
    List<Reservation> selectExpiryCandidates(@Param("now") LocalDateTime now,
                                             @Param("limit") Integer limit);

    /** 单预约过期 CAS；带 user_id 命中单分片。 */
    int expireByNo(@Param("userId") Long userId,
                   @Param("reservationNo") Long reservationNo,
                   @Param("now") LocalDateTime now);
